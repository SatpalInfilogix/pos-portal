$(document).ready(function () {
    ...
    if ($('.datanew').length > 0) {
        $('.datanew').each(function () {
            let $table = $(this); // Reference to the current table
            let $tableSearchContainer = $table.parent().parent().find('.search-input');
            ...
            $table.DataTable({
                ...
                initComplete: function (settings, json) {
                    // Use setTimeout to ensure filter is available
                    setTimeout(() => {
